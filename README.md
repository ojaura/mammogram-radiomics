...
A Radiomics workflow for extracting quantitative imaging features from mammography images (CBIS-DDSM) using PyRadiomics. The current goal is to account for breast-tissue density variations and to build a reproducible PyRadiomics pipeline for future machine-learning experiments.
...
To maintain licensing and file-size constraints:

    •    Mammography images are not included
    
    •    Repository focuses on pipeline structure, not dataset distribution
